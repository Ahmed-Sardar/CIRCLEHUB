...
const UserCard = ({user}) => {
  ...
  return (
    <div key={user} className='p-4 pt-6 flex flex-col justify-
    between w-72 shadow border border-gray-200 rounded-md' >
      ...
      <div className='text-center'>
        ...
        {user.bio && <p className='text-gray-600 my-2 text-center text-sm px-4'>{user.bio}</p> }
      </div>
      ...
      <div className='flex mt-4 gap-2'>
        {/* FOLLOW BUTTON */}
        <button onClick={handleFlollow} disabled={currentUser?.following.includes(user._id)} className="w-full py-2 rounded-md flex justify-center items-center gap-2 
